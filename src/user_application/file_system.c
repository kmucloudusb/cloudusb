...
{
        // Reserved Area
    if (sector == FAT_RESERVED_AREA_POSITION || sector == FAT_RESERVED_AREA_BACKUP_POSITION) {
        memcpy(buffer,
        ...
        // Fat Area
    else if (sector == FAT_FAT_AREA_POSITION || sector == FAT_FAT_AREA_BACKUP_POSITION) {
        memcpy(buffer,
               fat_area + (sector - FAT_FAT_AREA_POSITION) % FAT_FAT_AREA_FULL * FAT_SECTOR_SIZE,
               count * FAT_SECTOR_SIZE);
    }

        // Entries
    else {
        unsigned int cluster = (sector - FAT_ROOT_DIR_POSISTION) / FAT_CLUSTER_SIZE + FAT_ROOT_DIRECTORY_FIRST_CLUSTER;
